...
for (fileName in fileNames) {
  ...
  tryCatch(
    ...
    error = function(e){
      ...
      if (first) {
        disp_table <- as.data.frame(matrix(c(paste(fileName, ".txt", sep=''), R_sun, is_rotating), byrow=TRUE, nrow=1))
        first = FALSE
      } else {
        disp_table[nrow(disp_table)+1,] <- c(paste(fileName, ".txt", sep=''), R_sun, is_rotating)
      }
...
colnames(disp_table) = c("file_name", "r_sun", "rotating")
...
DT::datatable(complete_data[complete_data$rotating == 1,], rownames=FALSE)