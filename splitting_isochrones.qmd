...
```{r}
f_data <- read.csv("./data/clean-clusters/catalogues/NGC_6752.txt", sep=',')
...
p = f_data |>
  ...
  scale_y_reverse() +
  labs(y="G (brightness - absolute)") +
  scale_color_gradientn(colors=rainbow(10), na.value = "transparent")
...
max_g = max_min$max
min_g = max_min$min
num_bins = round(length(f_data$`Source ID`)/1000) # Enough points per bin for CLT
