...
for i in range(0,n_bins-1):
    for j in range(0,n_bins-1):
        ...
        if len(filtered_df) != 0:
            vx_bins.append(np.mean(filtered_df['vx']))
            vy_bins.append(np.mean(filtered_df['vy']))
            size_bins.append(len(filtered_df['x']))
        else:
            vx_bins.append(0)
            vy_bins.append(0)
            size_bins.append(0)
...
plt.quiver(X_bins, Y_bins, vx_bins, vy_bins, pivot="mid", width=0.0025, color=cm(norm(size_bins)))
cbar = plt.colorbar(sm)
cbar.set_label('# stars (proportion)')
plt.xlabel('Right Ascension (mas) relative to GC center')
plt.ylabel('Declination (mas) relative to GC center')
plt.show()
