...
# Accessing the file
gcPath = 'data/clean-clusters/filtered/'
...
for gcName in gcNames:
    gc_data = pd.read_csv(gcPath + gcName, header=0).dropna(subset=["point_bp_rp", "point_gmag"])
    gc_bp_rp = gc_data['point_bp_rp']

    r_pc = 1000*harris_data[harris_data["Name"]==gcName]["R_sun"].to_list()[0]
    gc_gmag = gc_data['point_gmag'] - 5*np.log10(r_pc/10)
